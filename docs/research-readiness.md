...
The [public-site benchmark](public-site-benchmark.md) adds seven one-step cases from three public demonstration sites. Its first three-run comparison found that reversing Laya's candidate order corrected a dropdown decision but regressed a TodoMVC filter, leaving total completion unchanged at 18/21. These evaluation-only cases broaden site coverage but do not add validated training or calibration labels, so the readiness gate still fails.
