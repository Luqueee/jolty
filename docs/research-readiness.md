...
A [seventh corpus](research-corpus.md#corpus-v6-and-multistep-probe) preserves the v5 training and calibration split and adds twenty validated decisions on two new test origins. The [fixed v5 head comparison](frozen-encoder-experiment.md#fixed-v5-heads-on-v6-and-multistep-flows) completed 17/20 live one-step tasks for each head and 18/20 for Laya. In two three-step public-page flows, the curated reference completed 6/6 measured tasks; both heads and Laya completed 3/6, all failing the first Sreenidhi field. This provides a multistep failure probe but does not establish representative throughput, reliable calibration, or a measurable advantage over Laya. Milestone 13 remains open and no research head is deployed.
