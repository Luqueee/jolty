...
The first proposed target is a decision pipeline below **25 ms**, excluding browser rendering and navigation. A later target is below **10 ms** on suitable hardware with an optimized Jolty model. The Laya baseline does not meet the first target on the measured CPU: its controlled fixture decision p50 was 84.1 ms and p95 was 104.2 ms. These are measured baseline results, not full browser-step or end-to-end test times. Actual browser wait time and full test duration need separate reporting.
...
- **Keep the model resident:** The Laya baseline loads one ONNX session and tokenizer, then reuses them for inference. The controlled cached load took 2.1 s; the initial bundle download is separate. Exclude one-time loading from per-step latency while reporting startup separately.
...
On the 14 controlled fixture decisions, CPU p50 was 0.47 ms for tokenization, 83.6 ms for ONNX inference, and 84.1 ms for decision latency. Candidate filtering p50 was 0.0033 ms on the same fixtures. Inference dominates this baseline; these small samples and fixtures do not establish real-site latency. Run `pnpm run benchmark:decision` for current measurements and per-case output.
