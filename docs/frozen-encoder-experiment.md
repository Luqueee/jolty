...
## Corpus v8 frozen-head iteration

V8 followed the [predeclared protocol](research-corpus.md#corpus-v8-protocol-fixed-before-model-scoring) and used corpus digest `aa9d1e4b8827547e43dfc7ab697de49d61943b9807654a91bc7b94200275595a`. A bounded nearby-label fallback recovered names for otherwise unnamed registration inputs; a native-select marker retained valid multiple-select training labels without changing the 388-feature head shape. The collector redacts visible public-page email addresses before persistence. The training recipe remained the pinned MiniLM encoder, 400 steps, learning rate 0.5, L2 0.01, and zero global action intercepts. Validation alone selected temperature 0.25 and threshold 0.919982. All 104 training labels entered fitting; the head matched 89/104 training and 49/52 calibration decisions.

| Measure | Frozen v8 head | Frozen v7 head | Laya | ChatGPT subscription reference |
| --- | ---: | ---: | ---: | ---: |
| Exact decisions on the same saved v8 test observations | 10/10 | 10/10 | 5/10 | 10/10 |
| Exact decisions and completed tasks on live v8 pages | 10/10 | — | 5/10 | — |
| Live decision latency p50 / p95 | 7.72 / 15.30 ms | — | 162.61 / 178.88 ms | — |

The v7 result is a read-only evaluation of its fixed weights and v7 temperature/threshold on the v8 saved test observations; it does not retrain v7. The subscription-backed teacher was `gpt-6-sol` and scored saved observations without browser execution. Each live head flow ran once; Laya had one warmup and one measured run per flow. The head's test top-choice Brier score was 0.055. Its validation threshold covered 6/10 test decisions, all correct; the fixed 0.9 diagnostic threshold covered the same six. The live head process peaked at 464,908,288 bytes RSS. Decision timings exclude model loading and page setup. The ten easy authored decisions on two sites cannot establish safe calibration or a general performance gain. In particular, v8 did not improve exact decisions over v7 on the same test set.

As a separate, already inspected regression diagnostic, v8 scored 20/21 saved v7 test decisions. It repeated the `learn-create-shadow` wrong click at confidence 0.998746, above the v8 validation threshold. This historical error independently blocks runtime integration; these v7 labels were not used to fit or select the v8 recipe.

`pnpm research multistep --suite heldout --policy zero --head-version 8 --runs 3` probes v8 on the two already inspected sites. It completed 3/6 measured tasks and 12/15 attempted steps, the same counts as v7. The product-catalog flow still chose the Search button before entering the query in all three runs, at confidence 0.895659; the correct target remained in Top-10. The form flow completed all three runs. This diagnostic rules out the hoped-for fix to that observed error. The frozen-head iteration stops here; the candidate remains outside the runtime and Milestone 13 remains open. A separate PEFT experiment needs a broader and harder independent evaluation design before it can support a deployment decision.
