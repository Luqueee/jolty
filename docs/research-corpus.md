...
GlobalSQA was replaced by Syntax before label authoring because its widgets use frames outside the main-document contract. Cases requiring targets outside Top-10, query-bearing URLs rejected by the privacy guard, or unsupported postconditions were excluded or revised before model scoring. These are live public pages and can change between collections. The matched frozen head and LoRA adapter have now been fitted on train and calibrated on validation, with the [fitting details](peft-experiment.md#fitting-run-before-held-out-scoring) recorded separately. The v9 test predictions remain unscored, and Milestone 13 remains open.
