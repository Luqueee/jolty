...
Model development remains separate from the browser runtime. The [frozen-encoder research experiments](frozen-encoder-experiment.md) use a Node.js script and an offline JSON corpus. The [PEFT experiment](peft-experiment.md) launches a separate Python process for parity, GPU memory smoke, and LoRA fitting; Python does not enter the normal browser or decision path. Neither experiment has produced a deployable candidate. The specification's Parquet and DuckDB workflow remains proposed for larger research datasets and a successful ONNX candidate. A dashboard and a separate inference service are not part of the initial design. Later [roadmap](roadmap.md) stages cover trace compilation and self-healing.
