...
The [frozen-encoder experiments](frozen-encoder-experiment.md) train a small head on separate public practice sites and compare it with Laya and a ChatGPT subscription reference on six reserved public-flow collections. The first candidate underperformed Laya; the next two each completed one more task than Laya on their small one-run collections. The unchanged third head then completed 7/10 tasks on another fresh collection versus Laya's 9/10. A predeclared 0.9 confidence gate passed two high-confidence errors in that probe. A newly trained fifth head tied Laya at 8/10 on two further origins, while its validation-selected threshold also passed two high-confidence errors. On a sixth collection, freezing the head's global action intercept yielded 9/10 live task completion versus 8/10 for the fitted-intercept variant and Laya. That one-task difference on ten authored flows does not establish a general benefit or a validated execution threshold, so the head remains outside the runtime.

The [v7 frozen-head experiment](frozen-encoder-experiment.md#corpus-v7-frozen-head-experiment) used a larger corpus with 89 trainable labels, 47 validation labels, and 21 held-out decisions on two new origins. In one live run it completed 19/21 one-step tasks versus Laya's 15/21, but its validation-selected threshold admitted a high-confidence wrong decision. This result needs independent sites, repeated runs, and multistep checks before it can establish a reliable gain or safe coverage.
...
The fixed v7 head was also measured on [two new three- and four-step flows](frozen-encoder-experiment.md#v7-head-on-two-new-multistep-sites) on origins outside all research corpora. Across three measured runs per flow, the curated reference completed 6/6 tasks, the head 3/6, and Laya 0/6. Both models failed the same catalog flow at its first step; the head completed the form flow in all runs. These repeated states expose a consistent failure pattern but are not an independent sample of six workflows.
