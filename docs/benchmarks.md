...
The [frozen-encoder experiments](frozen-encoder-experiment.md) train a small head on separate public practice sites and compare it with Laya and a ChatGPT subscription reference on four reserved public-flow collections. The first candidate underperformed Laya; the next two each completed one more task than Laya on their small one-run collections. The unchanged third head then completed 7/10 tasks on another fresh collection versus Laya's 9/10. A predeclared 0.9 confidence gate passed two high-confidence errors in that probe. No candidate has established a general benefit or a validated execution threshold, so the head remains outside the runtime.
