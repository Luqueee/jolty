...
test("keeps test cases on two independent HTTPS origins", () => {
  ...
  expect(new Set(freshTestCasesV7.map((entry) => entry.id)).size).toBe(
    freshTestCasesV7.length,
  );
  expect(
    new Set(freshTestCasesV7.map((entry) => new URL(entry.url).origin)),
  ).toEqual(
    new Set([
  ...
  for (const entry of freshTestCasesV7) {
    const url = new URL(entry.url);
    expect(entry.split).toBe("test");
...
test.skipIf(!live).each(freshTestCasesV7)(
