...
for (const model of plan.models) {
  const incomplete = model.id === "laya-browser-v10s";
  const rows = (await read(
    incomplete
      ? "artifacts/blind-v2-once/laya-browser-v10s-status-corrected.json"
  ...
  modelResults.push({
    ...
    exact: incomplete ? null : decisions.filter((row) => row.exact).length,
    accepted_correct: incomplete
      ? null
      : decisions.filter((row) => row.accepted && row.exact).length,
    accepted_wrong: incomplete
      ? null
      : decisions.filter((row) => row.accepted && !row.exact).length,
    unscorable:
      decisions.filter(
        (row) => row.failure === "unsupported_all_singleton_questions",
      ).length +
      (incomplete ? 0 : decisions.filter((row) => row.failure).length),
    scores_lost: decisions.filter(
      (row) => row.failure === "run_aborted_no_persisted_score",
    ).length,
    reference_flows_completed: incomplete
      ? null
      ...
      [...new Set(labels.map((row) => row.site))].map((site) => {
        ...
        return [
          site,
          {
            ...
            exact: incomplete ? null : subset.filter((row) => row.exact).length,
            accepted_wrong: incomplete
              ? null
              : subset.filter((row) => row.accepted && !row.exact).length,
          },
        ];
      }),
    ),
...
const report = {
  ...
  incident:
    "Frozen v10s scorer raised RuntimeError on the final all-singleton question before writing output. Earlier 16 in-memory scores were lost; no inference was repeated after labels were opened.",
  models: modelResults,
  teacher: {
...
await writeFile(
  "artifacts/blind-v2-once/comparison-final.json",
