...
const inputPath = "artifacts/blind-v2-once/observations.json";
const outputPath = "artifacts/blind-v2-once/teacher.json";
...
for (const sample of observations) {
  const state: BrowserState = {
    ...
    elements: sample.browser_state.elements.map((element) => ({
      ...
      value: element.native_select
        ? ((element as typeof element & { current_value?: string })
            .current_value ?? "")
        : undefined,
    })),
  };
